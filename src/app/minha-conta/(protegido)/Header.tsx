...
//
// No Dashboard (Início) o cabeçalho não é fixo: ele faz parte do bloco
// da hero (mesma cor navy) e rola junto com ela — testado com fundo
// sólido + fixo antes, mas a cor batia diferente da hero no meio do
// scroll (destacando a linha de corte); rolando junto, nunca aparece
// essa emenda.
export function Header({ nome, fotoUrl }: { nome: string; fotoUrl?: string | null }) {
  ...
  if (naHome) {
    return <div className="header header-hero-inline">{conteudo}</div>;
  }
