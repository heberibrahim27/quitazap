...
const PADROES_ESCOPO = [
  ...
  // cartão (achado em teste ao vivo, 09/09/2026) — sem isso, mensagens só
  ...
  /\bdivida\b/,
  /\bdividas\b/,
  // Sinônimos de dívida/empréstimo que resolverDivida/resolverPagamentoDivida
  // já sabem interpretar, mas que o guardião de escopo ainda não reconhecia —
  // sem essas palavras aqui, mensagens como "peguei um empréstimo de 3000,
  // pago 500 por mês" nunca chegavam nem no resolvedor local nem na IA,
  // caindo direto na resposta genérica de fora de escopo. Achado durante
